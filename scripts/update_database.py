...
EXCEL_FILE = os.path.join(BASE, "Maize Pathogen.xlsx")
...
# ── Excel Sheet Map ───────────────────────────────────────────────
# Sheet name -> internal category. Unified 13-column layout:
# 1=序号, 2=学名, 3=病害EN, 4=病害CN, 5=界, 6=门, 7=纲, 8=目,
# 9=科, 10=属, 11=种, 12=TaxID, 13=文献
EXCEL_SHEETS = {
    "bacteria": "bacteria",
    "virus": "viruses",
    "fungi and Oomycota": "fungi",
}
EXCEL_NAME_COL = 2
EXCEL_DISEASE_EN_COL = 3
EXCEL_DISEASE_CN_COL = 4
EXCEL_TAXONOMY_COLS = (5, 6, 7, 8, 9, 10, 11)
EXCEL_TAXID_COL = 12
EXCEL_DATA_START = 2
...
# ═══════════════════════════════════════════════════════════════════
log("=" * 60)
log("STEP 1/8: Parsing Maize Pathogen.xlsx")
log("=" * 60)
...
for sheet_name, cat in EXCEL_SHEETS.items():
    if sheet_name not in wb.sheetnames:
        log(f"  ⚠ Sheet '{sheet_name}' not found in {os.path.basename(EXCEL_FILE)}")
        continue
    ws = wb[sheet_name]

    n_species = 0
    for r in range(EXCEL_DATA_START, ws.max_row + 1):
        name = ws.cell(r, EXCEL_NAME_COL).value
        if name is None:
            continue
        name = str(name).strip()
        ...
        disease_cn = str(ws.cell(r, EXCEL_DISEASE_CN_COL).value or "").strip()
        disease_en = str(ws.cell(r, EXCEL_DISEASE_EN_COL).value or "").strip()
        raw_taxid = ws.cell(r, EXCEL_TAXID_COL).value
        taxid = extract_taxid(raw_taxid)
        if not taxid:
            # Keep entries without a valid NCBI TaxID in the species catalog,
            # but mark them so sequence download is skipped later.
            taxid = str(raw_taxid).strip() if raw_taxid else "NO_TAXID"
            log(f"  ⚠ No valid TaxID for row {r}: {name} (kept as {taxid})")

        # Parse taxonomy lineage
        kc, pc, cc, oc, fc, gc, sc = EXCEL_TAXONOMY_COLS
        tax = {
        ...
        species_records.append({
            "tax_id": taxid,
            "has_ncbi_taxid": bool(extract_taxid(raw_taxid)),
    ...
    log(f"  {sheet_name}: {n_species} species parsed")
...
for cat in ["bacteria", "viruses", "fungi"]:
    taxids = [r["tax_id"] for r in species_records
              if r["category"] == cat and r.get("has_ncbi_taxid")]
    taxid_path = os.path.join(TAX_DIR, f"taxids_{cat}.txt")
    with open(taxid_path, "w") as f:
        f.write("\n".join(taxids))
    log(f"  ✓ Saved: {taxid_path} ({len(taxids)} valid NCBI TaxIDs)")
...
existing_fasta = os.path.join(SEQ_DIR, "maize_pathogens_all.fasta")
if SKIP_DOWNLOAD and os.path.exists(existing_fasta):
    log("  --skip-download: loading existing FASTA instead of downloading")
    with open(existing_fasta) as f:
        current = None
        for line in f:
            if line.startswith(">"):
                if current:
                    all_seqs[current[0]] = current[1]
                header = line[1:].strip()
                current = [header, ""]
    ...
    for hdr in all_seqs:
        parts = hdr.split("|")
        if len(parts) >= 3:
            downloaded_taxids.add(parts[0])
    log(f"  Loaded {len(all_seqs)} sequences from existing FASTA")
else:
    for i, rec in enumerate(species_records):
        ...
        if not rec.get("has_ncbi_taxid"):
            log(f"  [{i+1}/{total_species}] {cat}: {sp[:50]} (no NCBI TaxID, kept in catalog, skipped download)")
            continue
...
# Find makeblastdb
BLAST_CANDIDATES = [
    os.path.join(BASE, "tools", "ncbi-blast-2.17.0+", "bin", "makeblastdb"),
...
BLAST_BIN = next((p for p in BLAST_CANDIDATES if os.path.exists(p)), None)
